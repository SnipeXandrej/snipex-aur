...
do
    ...
    AURDEST=$AUR_DIR aur sync --noview --chroot -D ${CHROOT_DIR} --rebuild -d snipex $(echo "${$(cat ${PKG_LIST} | grep -E "^[^#]" | head -$i | tail +$i)%#*}") --makepkg-conf ${MAKEPKG_CONF} --margs --skipinteg --noconfirm
done
