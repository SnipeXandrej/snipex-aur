...
do
    ...
    AURDEST=$AUR_DIR aur sync --noview --chroot -D ${CHROOT_DIR} -d snipex "$(cat ${PKG_LIST} | sed 's/#.*//' | grep . | head -$i | tail +$i)" --makepkg-conf ${MAKEPKG_CONF} --margs --skipinteg --noconfirm
done
